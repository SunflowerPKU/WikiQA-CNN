...
class BilinearLR(object):
    ...
    def predict(self, ldata, rdata):
        ...
        return p_y_given_x
    #交叉熵作为损失函数（避免因为使用梯度下降法求解w,b时间过长）
    def get_cost(self, y):
        # cross-entropy loss
...
def train_qacnn(datasets, 
    ...
    train_set_lx_orig, train_set_rx_orig, train_set_y_orig = train_set_orig[:, :lsize], train_set_orig[:, lsize:lsize+rsize], np.asarray(train_set_orig[:,-1], dtype="int32")
    val_set_lx, val_set_rx, val_set_y = val_set[:, :lsize], val_set[:, lsize:lsize+rsize], np.asarray(val_set[:,-1], dtype="int32")
    test_set_lx, test_set_rx, test_set_y = test_set[:, :lsize], test_set[:, lsize:lsize+rsize], np.asarray(test_set[:,-1], dtype="int32")
    ...
    while (epoch < n_epochs):
        ...
        for pr in ypred:
            test_preds.append(pr)

    return train_preds, dev_preds, test_preds
